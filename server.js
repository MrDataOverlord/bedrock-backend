// server.js (all-in-one, SMTP-first email, robust links, more Stripe events)
...
const {
  ...
  SMTP_USER = "",
  SMTP_PASS = "",
  MAIL_FROM = "",       // if blank, will default to SMTP_USER

  // 2) Resend (fallback if SMTP not provided)
  RESEND_API_KEY = "",

  // Registration/Reset link bases:
  // You can supply either a base path (…/account) or the full endpoint (…/set-password)
  REG_URL_BASE = "https://www.nerdherdmc.net/account",
  RESET_URL_BASE = "https://www.nerdherdmc.net/account",
} = process.env;
...
/* ---------- Email: SMTP first, then Resend, then console ---------- */
...
async function sendEmail({ to, subject, html }) {
  ...
  if (RESEND_API_KEY) {
    const r = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({ from: MAIL_FROM || "NerdHerd Utilities <no-reply@nerdherdmc.com>", to: [to], subject, html })
    });
...
/* ---------- Link builder that avoids /set-password/set-password ---------- */
const buildActionLink = (base, endpoint, rawToken) => {
  const b = String(base || "").replace(/\/$/, "");
  const tail = `/${endpoint}`;
  const hasEndpoint = b.toLowerCase().endsWith(tail);
  const url = hasEndpoint ? `${b}?token=${encodeURIComponent(rawToken)}`
                          : `${b}${tail}?token=${encodeURIComponent(rawToken)}`;
  return url;
};
...
async function sendRegistrationEmail(email, userId) {
  ...
  const url = buildActionLink(REG_URL_BASE, "set-password", raw);
  await sendEmail({
    to: email,
...
async function sendResetEmail(email, userId) {
  ...
  const url = buildActionLink(RESET_URL_BASE, "reset-password", raw);
  await sendEmail({
    to: email,
...
async function sendPaymentFailedEmail(email) {
  ...
  await sendEmail({
    to: email,
    subject: "Payment failed — NerdHerd Utilities",
    html: `<p>We couldn’t process your latest payment and your premium access is on hold.</p>
           <p>Please update your payment method in the Stripe checkout or start a new checkout from your account.</p>
           <p>If you no longer remember your password, you can request a reset from the sign-in screen.</p>`
  });
}
      ...
      switch (event.type) {
        case "checkout.session.completed": {
          ...
          if (!orgId && purchaserEmail) {
            ...
            if (!user) {
              user = await prisma.user.create({ data: { email: purchaserEmail } }); // no password yet
              ...
              // Only send set-password if user still has none
        ...
        case "invoice.payment_failed": {
          ...
          if (subId) {
            ...
            try { await sendPaymentFailedEmail(email); } catch {}
          }
          break;
        }
...
/* =========================
   Health & Home
========================= */
const HEALTH = (_req, res) => res.json({ ok: true, env: NODE_ENV, ts: new Date().toISOString() });
app.get("/healthz", HEALTH);
app.get("/health", HEALTH); // convenience

app.get("/", (_req, res) => {
  res.type("text/plain").send(
`Bedrock Backend

Status: /health or /healthz
...
app.post("/billing/checkout_public", async (req, res) => {
  try {
    ...
    // Pre-issue registration email (webhook also handles it if needed)
    try {
      const hasPassword = !!(await prisma.user.findUnique({ where: { email }, select: { passwordHash: true } }))?.passwordHash;
      if (!hasPassword) await sendRegistrationEmail(email, user.id);
    } catch {}
