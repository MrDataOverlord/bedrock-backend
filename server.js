...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      // 3) Invoice paid → make sure it's active and extend
      if (event.type === "invoice.payment_succeeded") {
        const inv = event.data.object;
        const subId = inv?.subscription;
        // Stripe invoices carry the billing period on the first line usually
        const maybeEnd =
...
app.post("/billing/checkout", async (req, res) => {
  try {
    ...
    // Where do we send users after checkout?
    // Use provided APP_DOMAIN if set; otherwise reconstruct from the request
    const base =
      appBase ||
...
// ---------- tiny success/cancel pages (no getToken needed) ----------
...
<script>
  // Let an opener (mobile/web app) know it can refresh entitlements
